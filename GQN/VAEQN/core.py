...
class InferenceCore(nn.Module):
    def __init__(self):
        ...
        self.downsample_x = nn.Conv2d(1, 1, kernel_size=4, stride=4, padding=0, bias=False)
        self.upsample_v = nn.ConvTranspose2d(8, 8, kernel_size=16, stride=16, padding=0, bias=False)
        ...
        self.core = Conv2dLSTMCell(1+8+256+2*128, 128, kernel_size=5, stride=1, padding=2)
        
    def forward(self, x, v, r, c_e, h_e, h_g, u):
        x = self.downsample_x(x)
        v = self.upsample_v(v.view(-1, 8, 1, 1))
...
class GenerationCore(nn.Module):
    def __init__(self):
        ...
        self.upsample_v = nn.ConvTranspose2d(8, 8, kernel_size=16, stride=16, padding=0, bias=False)
        self.upsample_r = nn.ConvTranspose2d(256, 256, kernel_size=16, stride=16, padding=0, bias=False)
        self.core = Conv2dLSTMCell(8+256+1, 128, kernel_size=5, stride=1, padding=2)
